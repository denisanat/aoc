...
for (let i = 0; i < lineas.length; i++) {
    let sets = lineas[i].split(":")[1].split(";");
    maxColor = [0, 0, 0];
    for (let set of sets) { 
        for (let bola of set.split(",")) {
            let valor_color = bola.trim().split(" ");
            if (valor_color[1] === "red" && Number(valor_color[0]) > maxColor[0])
                maxColor[0] = Number(valor_color[0]);
            if (valor_color[1] === "green" && Number(valor_color[0]) > maxColor[1])
                maxColor[1] = Number(valor_color[0]);
            if (valor_color[1] === "blue" && Number(valor_color[0]) > maxColor[2])
                maxColor[2] = Number(valor_color[0]);
        }
    }
    suma += maxColor[0] * maxColor[1] * maxColor[2];
}
